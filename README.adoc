...
Inserting or deleting `dbg`/`dbgn`/`clog`/`clogn` manually is very painful. As an Emacs
user I wrote `debux.el` for Emacs CIDER for my convenience. It's not perfect but beter
than nothing. If you find it useful, append the following `debux.el` (which is in project
root folder as well) to the `~/.emacs.d/init.el`.
  
### `debux.el`

Refer to `debux.el` source code https://github.com/philoskim/debux/blob/master/debux.el[here]
...
When you double-click on one of the open parentheses and the following string is `dbgn` or
`clogn`, it will be deleted.
