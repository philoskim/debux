...
* *in production*, your use of *debux* macros will have zero runtime and compile time
   cost,
...
:sectnums!:
### Recommendations on use of *Debux* macros

You should use `clog`/`clogn` instead of `dbg`/`dbgn` in the browser console window,
because `clog`/`clogn` uses the `console.log` function of browser's developer tools to
style the form. You can see its effect <<style-option, here>>.

However, you should use `dbg`/`dbgn` instead of `clog`/`clogn` on Node.js JavaScript
...
Of course, you should use the `clog`/`clogn` macros in
