...
NOTE: The clojure source codes are the Clojure data structures as well, which is known as
      _homoiconcity_. When the debux macros traverse the Clojure source code tree, they
      enter the lists or vectors within the tree but don't enter the maps or sets within
      the tree. So the codes within the above map `m` aren't printed in the middle of the
      evaluated results, because they are wrapped inside the map `m`.
...
* The `clog`/`clogn` of `debux` supports
  ...
  `0.5.9`.
+
image::cljs-devtools.png[title="cljs-devtools printing example", width=600]
...
+
image::chrome-devtools-settings.png[title="Enable custom formatters on Chrome DevTools", width=800]
