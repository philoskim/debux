...
** The temporal _turn-off_ version of Debux macros added. See <<temporal-turn-off, here>>
   for the details.
** The safe debugging in multi-threads now supported. See <<msg-option, here>> for the
   example.
...
If the above `String` option and this `:msg` option both exist, `:msg` option has higher
precedence.
