...
* v0.7.3
** `pass:[some->]` and `pass:[some->>]` support in `dbg` and `clog` added. See
   <<some-threading-macros, here>> for the details.
** `pass:[cond->]` and `pass:[cond->>]` support in `dbg` and `clog` added. See
...
following eight special cases (`pass:[->]`, `pass:[->>]`, `pass:[some->]`,
`pass:[some->>]`, `pass:[cond->]`, `pass:[cond->>]`, `let`, `comp`).
...
The support for the thread macro `pass:[some->]` and `pass:[some->>]` is added to `dbg`
and `clog`.
...
### Debugging the thread macro `pass:[cond->]` or `pass:[cond->>]`

The support for the thread macro `pass:[cond->]` and `pass:[cond->>]` is added to `dbg`
and `clog`.
