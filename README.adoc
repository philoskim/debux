...
* v0.6.0
** Changed: The <<_number_option, _number_>> option of *debux* macros is now applied to
   not only `seq` data type but also all the `coll` data types. (Previously, the _number_
   option was applied to only `seq` data type.)
*** This change will prevent the deeply nested massive `coll` data types from printing 
    too much contents.
...
[[set-print-length]]
If you want to change the default number of 100, use `set-print-length!` function like
this.
