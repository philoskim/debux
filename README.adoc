...
* The `clog`/`clogn` of `debux` supports
  ...
  `0.5.9`.
+
image::cljs-devtools.png[title="cljs-devtools' example", width=600]

* The <<two-libraries, *development*>> version `debux` library already includes
  `cljs-devtools 0.9.11` version on its dependencies.
+
NOTE: Of course, you can include the newer version of `devtools` on the `:dependencies`
...
link:https://github.com/binaryage/cljs-devtools/blob/master/docs/installation.md[devtools'
installition guide].
+
[listing]
.project.clj
...
+
image::chrome-devtools-settings.png[title="Enable custom formatters", width=800]
