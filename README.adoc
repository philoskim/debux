...
### Debugging thread macro `pass:[some->]` or `pass:[some->>]`

When I encountered the following code and wanted to debug it sequentially one by one, I
used to transform it temporarily like this. The following example code is very short, but
sometimes can be very long.
...
However, it is very annoying. Of course, the same form can be debugged by using `dbgn`,
but the printing result is not very intuitive as you can see.
...
| G__1947 =>
|   {:a 1}
| (clojure.core/nil? G__1947) =>
|   false
| (:b G__1947) =>
|   nil
| (if (clojure.core/nil? G__1947) nil (:b G__1947)) =>
|   nil
...
| (clojure.core/nil? G__1947) =>
|   true
| (if (clojure.core/nil? G__1947) nil (inc G__1947)) =>
|   nil
| (clojure.core/let [G__1947 {:a 1} G__1947 (if (clojure.core/nil? G__19 ... =>
|   nil
....

So `pass:[some->]` or `pass:[some->>]` macro support is added to `dbg` and `clog` for the
...
within `dbg` and `clog` internally behave exactly in the same way as in the thread macro
`pass:[->]` and `pass:[->>]`
