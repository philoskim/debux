...
*In development*, you want to include the `philoskim/debux` library. When you use *debux*
 macros and functions from this library, it will emit debugging messages to REPL window or
 chrome DevTools' console.

*In production*, you want to include the `philoskim/debux-stubs` library. This has the
 same public API as `philoskim/debux`, but the macros simply expand to the given original
 form.

With this setup, your use of *debux* macros will have zero runtime and compile time cost
in production builds, and are able to be turned off at development time too via the
`set-debug-mode!` function. This ensures that you can leave your code instrumented at all
times, but not pay any costs in production.
...
* v0.4.4
** Added: The new `debux-stubs` library is added for production mode support. See
   <<two-libraries, here>> for details.
** Added: `set-debug-mode!` is added. See <<debux-config, here>> for details.
** Added: `set-ns-whitelist!` and `set-ns-blacklist!` are added. See <<debux-config,
   here>> for details.

* v0.4.3 
** Changed: The same duplicate evaluated results are not printed by default. See
   <<dup-option>> for detalis.
** Enhanced: The readability is enhanced for the looping constructs such as `map`,
   ...
   happens. See <<enhanced-readability, here>> for examples.
...
** Added: `set-print-seq-length!` function is added to change the default number when
   printing `seq` data type. See <<set-print-seq-length, here>> for example.
** Added: `dbg` can be used inside `dbgn` or vice versa. `clog` can be used inside `clogn`
   or vice versa. See <<mixed-use>> for details.
...
image::register-macros.png[title="register-macros! example", width=700]
