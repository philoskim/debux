...
Refer to <<register-macros>> about details.


## Change logs

* Version 0.3.3
** Fixed: `when` listed in `:let-type` by mistake is deleted.

* Version 0.3.2
** Fixed: The `dbgn/clog` related several bugs on ClojureScript are removed.
...
[#register-macros]
### How to register your own macros in using `dbgn` macro

* If you have some error when analyzing some source code while using `dbgn` macro,
...
* I tried to implement `register-macros!` in ClojureScript in several ways but all failed
  ...
  states(`debux.cs.macro-types/macro-types*` in my source code) in ClojureScript run-time
