...
TIP: I recommend that you should use `clog` instead of `dbg` in the browser console
     window, because `clog` macro uses the `console.log` fuction of browser's developer
     tools to style the form.
...
* Version 0.3.0
** Added: `dbgn`/`clogn` debugging macros on Clojure and ClojureScript ard added.
...
** Other macros such as `defn` which has a binding vector can have problem because they
   have bindnig symbols which must not be evaluated. In case of special forms and those
   macros in `clojure.core` namespace, `degn`/`clogn` macro can handle them appropriately.
...
* Legend: `O` (supported), `△` (limited support), `X` (not supported)

TIP: The evaluated results of `dbgn` in ClojureScript go not only to the browser REPL but
also to the browser's DevTool console window. So use `dbgn` instead of `clogn`, when
debugging in ClojureScript the form which includes `recur` form.
...
* If you have some error when analyzing some source code using `dbgn`/`clogn` macro, first
...
window and the browser's console window. When you evaluate `clog`/`clogn` macro in your
