...
* Version 0.3.6
** Added: limited support for the form including `recur`. Refer to <<recur-support>>.


* Version 0.3.5
** Fixed: redundant multiple vertial bar display error by an exception thrown in
   evaluating `dbgn`.
...
IMPORTANT: If you use `dbgn` in `defn`/`defn-`/`fn` ~ `recur` form, you will have the
following exception. This is inevitable, due to the implementation restriction.
...
TIP: However, if you temporarily replace `recur` to `function name` itself, you can debug
the form as follows. *Be careful* not to forget to recover `function name` itself to `recur`
after debugging.
...
|      clojure.core/reify catch clojure.core/loop clojure.core/import
|      clojure.core/defprotocol throw},
|    :case-type #{clojure.core/case},
|    :skip-arg-2-3-type #{clojure.core/areduce clojure.core/amap},
|    :skip-arg-1-type #{set!},
|    :let-type
