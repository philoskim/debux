...
NOTE: You can see _Change logs since v0.3.0_
...
Inserting or deleting `dbg`/`dbgn`/`clog`/`clogn` manually is very painful. I am a Emacs
user so I made `debux.el` for Emacs CIDER for convenience. If you find it useful, append
the following file (which is in project root folder as well) to the `~/.emacs.d/init.el`
...
#### Inserting or deleting pass:q[`(dbg ...)`]/pass:q[`(clog ...)`]

When you double-click on one of the open parentheses and the following string is not 
`dbg` or `clog`, it will be inserted.

* The `v` of the following example is the cursor position.
...
When you double-click on one of the open parentheses and the following string is `dbg` or
`clog`, it will be deleted.
...
#### Inserting or deleting pass:q[`(dbgn ...)`]/pass:q[`(clogn ...)`]
...
When you double-click on one of the open parentheses while pressing `<Ctrl>` key and the
following string is `dbgn` or `clogn`, it will be deleted.
