...
NOTE: In the above example, `dbg` doesn't get the `:line` number information from the
      Clojure compiler, so it is omitted. I don't know why the Clojure compiler doesn't
      provide the line number informaton through the code of `(:line (meta &form))` within
      the thread-last macro pass:q[`->>`] in this situation. If anyone knows about it,
      please let me know.
...
image::clog-1.png[title="clog and clogn example", width=650]
...
image::clog-2.png[title="Predefined style example", width=700]
...
image::clog-3.png[title="User-defined style example", width=650]
...
image::clog-4.png[title=":once option example", width=700]
...
image::clog-5.png[title=":js option example", width=800]
